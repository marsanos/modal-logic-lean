...
/-!
# Completeness of Logic_2 with respect to Dual Semantics

This file proves the completeness theorem for logic_2:
  ∀ φ, dual_valid φ → TwoProof φ

## Main Results (PROVEN - no sorry)
- `existence_nworld`: For n-worlds with ◇B, accessible world exists with B
- `existence_pworld`: For p-worlds with □B, accessible world exists with B
- `unbox_dia_consistent`: Consistency of unbox(Γ) ∪ {B} when ◇B ∈ Γ
- `undia_box_consistent`: Consistency of undia(Γ) ∪ {B} when □B ∈ Γ
- `logic_2_complete`: Main completeness theorem

## TODO: All auxiliary lemmas (currently with sorry)

### Meta-theoretic foundations
- `lindenbaum`: Every consistent set extends to a maximally consistent set
  (requires classical choice/Zorn's lemma)
- `box_or_dia`: ⊢ □φ ∨ ◇φ (characteristic axiom of logic_2, follows from ax_kn2)
- `unprovable_consistent`: ¬(⊢ φ) → {¬φ} is consistent
  (contrapositive of soundness)

### Properties of maximally consistent sets
- `max_consistent_closed`: Maximal sets closed under modus ponens
- `max_consistent_no_contra`: No contradictions in maximal sets
- `box_conj_in_max`: □ distributes over conjunction in maximal sets
- `box_conjList_in_max`: Extension to lists
- `dia_iff_not_box_not`: ◇φ ↔ ¬□¬φ in maximal sets
- `box_iff_not_dia_not`: Dual interdefinability

### Proof-theoretic lemmas
- `conj_implies_last`: List manipulation for isolating formulas
- `inconsistency_implies_box_neg`: Derive □¬B from inconsistency
- `inconsistency_implies_dia_neg`: Dual version

### The Truth Lemma
- `truth_lemma`: Satisfaction ↔ membership in canonical model
  (provable by induction on formula structure)

All these theorems should be provable from the definitions and axioms of the proof system.
-/
...
-- Helper function to build conjunction from a list of formulas
def conjList : List (ModalFormula α) → ModalFormula α
  | [] => ⊤
  | [φ] => φ
  | φ :: rest => φ ∧ conjList rest

-- A set of formulas is consistent if we cannot derive a contradiction from it
-- Γ is consistent if there is no finite subset Γ' such that ⊢ ¬(∧Γ')
def Consistent (Γ : Set (ModalFormula α)) : Prop :=
  ¬∃ (Γ' : List (ModalFormula α)),
    (∀ φ, φ ∈ Γ' → φ ∈ Γ) ∧ TwoProof (¬(conjList Γ'))

-- A set of formulas is maximally consistent if it is consistent and
-- adding any formula not in the set makes it inconsistent
def MaximallyConsistent (Γ : Set (ModalFormula α)) : Prop :=
  Consistent Γ ∧
  ∀ φ, φ ∉ Γ → ¬Consistent (Γ ∪ {φ})

-- Helper: check if a set contains ax_K ∧ ax_N
def ContainsAxKN (Γ : Set (ModalFormula α)) : Prop :=
...
theorem max_consistent_no_contra : ∀ (Γ : Set (ModalFormula α)) (φ : ModalFormula α),
  ...
  -- Γ is consistent
  have hCons := hMax.1
  -- Unfold the definition of consistency
  unfold Consistent at hCons
  -- We'll derive a contradiction by showing there exists a finite subset that proves ⊥
  -- Consider the list [φ, ¬φ]
  apply hCons
  let witness := (List.cons φ (List.cons (¬φ) List.nil))
  use witness
  constructor
  · -- Show all elements are in Γ
    intro ψ hψ
    -- hψ says ψ is in the list [φ, ¬φ]
    show ψ ∈ Γ
    unfold witness at hψ
    cases hψ with
    | head => exact hφ
    | tail _ hψ' =>
      cases hψ' with
      ...
      | tail _ hψ'' => cases hψ''
  · -- Show ⊢ ¬(φ ∧ ¬φ)
    -- witness = [φ, ¬φ], so conjList witness = φ ∧ conjList [¬φ] = φ ∧ ¬φ
    show TwoProof (¬conjList witness)
    unfold witness
    -- conjList (φ :: ¬φ :: []) = φ ∧ conjList (¬φ :: []) = φ ∧ ¬φ
    change TwoProof (¬(φ ∧ conjList (List.cons (¬φ) List.nil)))
    -- conjList [¬φ] = ¬φ
    change TwoProof (¬(φ ∧ ¬φ))
    -- We need to prove this is a theorem in TwoProof
    -- ¬(φ ∧ ¬φ) is a CPL theorem
    apply TwoProof.classical
    -- This is the sequent: ∅ ⊢ ¬(φ ∧ ¬φ)
    apply CPLSeqProof.neg_r
    -- Need to prove: φ ∧ ¬φ ⊢ ∅, i.e., φ ∧ ¬φ leads to contradiction
    apply CPLSeqProof.and_l
    -- Now we have: φ ::ₘ ¬φ ::ₘ ∅ ⊢ ∅
    -- Both φ and ¬φ are on the left, so we can use identity since
    -- we can view this as: φ ::ₘ ¬φ ::ₘ ∅ ⊢ ∅
    -- But we need φ on both sides. Actually, if φ and ¬φ are both on left,
    -- we can move ¬φ to derive φ on the right:
    -- Apply neg_l on ¬φ:  need φ ::ₘ ∅ ⊢ φ ::ₘ ∅
    have h : CPLSeqProof (φ ::ₘ ∅ ⊢ φ ::ₘ ∅) := by
