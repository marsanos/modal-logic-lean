...
namespace Modal.ProofSystems.K
...
inductive proof {Atom : Type} : Set (Formula Atom) → Formula Atom → Prop where
  ...
  | nec {Γ : Set (Formula Atom)} {φ : Formula Atom}
      (h_prem : proof Γ (Rules.nec φ).premise) :
      proof Γ (Rules.nec φ).conclusion
  | k {Γ : Set (Formula Atom)} {φ ψ : Formula Atom} :
      proof Γ (Axioms.k φ ψ)

instance proof_system (Atom : Type) : Logic.ProofSystem :=
  { form := Formula Atom,
    entails := proof }

end Modal.ProofSystems.K
